...
public class ImageUtil {
     ...
     *            设置新的图像高度
     */
    public static void scaleEq(String srcImageFile, String result, int _width, int _height) {
        try {
            ...
            float bili = 0f; //放大或者缩小的比例
            float widthBili = Float.parseFloat(((new BigDecimal(_width)).divide(new BigDecimal(width),4,BigDecimal.ROUND_HALF_UP)).toString());
            float heightBili = Float.parseFloat(((new BigDecimal(_height)).divide(new BigDecimal(height),4,BigDecimal.ROUND_HALF_UP)).toString());

            bili = widthBili > heightBili ? heightBili : widthBili;
            if (bili == 0) {
                bili = 1;
            }
            scale(srcImageFile, result, bili, true);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//		scaleEq("d:/Koala3.jpg", "d:/Koala3.jpg", 100, 100);
        String fileName = "asdfsdf.sdfdsf.sdf";
